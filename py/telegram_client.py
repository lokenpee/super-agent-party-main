...
class TelegramClient:
    ...
    async def run(self):
        ...
        # 从全局设置读取代理，解决国内无法直连 Telegram API 的问题
        if not self.proxy:
            try:
                settings = await load_settings()
                proxy = settings.get("systemSettings", {}).get("proxy", "")
                self.proxy = proxy if proxy else None
            except Exception:
                self.proxy = None
        logging.info(f"Telegram 代理设置: self.proxy = {self.proxy}")
    ...
    async def _send_omni_voice(self, chat_id: int, audio_data: bytes, is_opus: bool):
        ...
        try:
            ...
            async with self.session.post(url, data=data, **self._proxy_kwargs()) as resp:
                if resp.status != 200:
    ...
    async def _send_text(self, chat_id: int, text: str, reply_to_msg_id: Optional[int] = None):
        ...
        async with self.session.post(url, json=payload, **self._proxy_kwargs()) as resp:
            if resp.status == 200:
                return # 发送成功
            ...
            await self.session.post(url, json=payload, **self._proxy_kwargs())

    async def _send_photo(self, chat_id: int, image_url: str):
        # 先下载
        async with self.session.get(image_url, **self._proxy_kwargs()) as resp:
            if resp.status != 200:
                return
            img_bytes = await resp.read()
        #  multipart 上传
        ...
        await self.session.post(url, data=data, **self._proxy_kwargs())
    ...
    def _proxy_kwargs(self) -> dict:
        # proxy 已在 session 级别设置，无需 per-request 传参
        return {}
